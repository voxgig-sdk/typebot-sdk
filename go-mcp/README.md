...
two agent tools — `typebot_list` and `typebot_load` — built on the
...
{ "entity": "billing" }
{ "entity": "billing", "query": { } }

// typebot_load: one record by id
{ "entity": "analytics", "query": { "id": 1 } }
```
...
4. **Restart Claude Code.** The `typebot_list` and `typebot_load` tools now appear
   in new sessions. Ask the agent to *"list billing using typebot"*
   and it calls `typebot_list` with `{"entity":"billing"}`.
...
| Tool | Args | Returns |
|------|------|---------|
| `typebot_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `typebot_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
analytics | billing | folder | result | typebot | workspace
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"typebot_load","arguments":{"entity":"analytics","query":{"id":1}}}}'
```
