...
class FtCtrl extends TypebotControl
{
...
class FtClient
{
    ...
    private TypebotUtility $utility;

    public function __construct(TypebotUtility $utility, array $options)
    {
    ...
    public function get_utility(): TypebotUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TypebotContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TypebotContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TypebotConfig::make_feature($name);
    ...
    private static function buildUrl(TypebotSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TypebotError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TypebotResponse($fetched);
        }
    ...
    private function fail(TypebotContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TypebotContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
