...
import (
	...
	omni "github.com/voxgig-sdk/typebot-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TypebotError); is {
				out := map[string]any{
					"name":    "TypebotError",
...
func omniCtx(arg any, client *sdk.TypebotSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TypebotSDK); is {
			client = live
