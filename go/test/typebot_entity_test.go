...
func TestTypebotEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		typebotRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath([]any{"new", "typebot"}, setup.data), "typebot_ref01"))

		typebotRef01DataResult, err := typebotRef01Ent.Create(typebotRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		typebotRef01Data = core.ToMapAny(typebotRef01DataResult)
		...
		typebotRef01ResdataUp0 := core.ToMapAny(typebotRef01ResdataUp0Result)
		...
		typebotRef01DataDt0LoadResult := core.ToMapAny(typebotRef01DataDt0Loaded)
