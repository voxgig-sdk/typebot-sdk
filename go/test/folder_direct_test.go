...
func TestFolderDirect(t *testing.T) {
	t.Run("direct-list-folder", func(t *testing.T) {
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-folder", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-folder", func(t *testing.T) {
		setup := folderDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-folder", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type folderDirectSetupResult struct {
...
func folderDirectSetup(mockres any) *folderDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"TYPEBOT_TEST_FOLDER_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["TYPEBOT_TEST_FOLDER_ENTID"]; ok {
		...
		return &folderDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &folderDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
