...
describe('WorkspaceDirect', async () => {
  ...
  test('direct-load-workspace', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-workspace', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TYPEBOT_TEST_WORKSPACE_ENTID']
