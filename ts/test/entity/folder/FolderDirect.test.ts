...
describe('FolderDirect', async () => {
  ...
  test('direct-load-folder', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-folder', setup.live)) return
  ...
  test('direct-list-folder', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-folder', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TYPEBOT_TEST_FOLDER_ENTID']
