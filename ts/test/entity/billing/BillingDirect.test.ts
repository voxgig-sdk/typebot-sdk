...
describe('BillingDirect', async () => {
  ...
  test('direct-load-billing', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-billing', setup.live)) return
  ...
  test('direct-list-billing', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-billing', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TYPEBOT_TEST_BILLING_ENTID']
