...
class TypebotTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TypebotTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TypebotTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TypebotTestRunner.getenv("TYPEBOT_TEST_LIVE")
        override = TypebotTestRunner.getenv("TYPEBOT_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TypebotTestRunner.getenv(key)
        ...
        explain = TypebotTestRunner.getenv("TYPEBOT_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["TYPEBOT_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if TypebotTestRunner._test_control is not None:
            return TypebotTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TypebotTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TypebotTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TypebotTestRunner.load_test_control()
...
def load_env_local():
    TypebotTestRunner.load_env_local()
...
def entity_list_to_data(lst):
    return TypebotTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TypebotTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TypebotTestRunner.load_test_control()


def live_client_options():
    return TypebotTestRunner.live_client_options()


def live_delay_ms():
    return TypebotTestRunner.live_delay_ms()
