...
describe("TypebotDirect", function()
  it("should direct-list-typebot", function()
    local setup = typebot_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-typebot", setup.live and "live" or "unit")
  ...
  it("should direct-load-typebot", function()
    local setup = typebot_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-typebot", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function typebot_direct_setup(mockres)
