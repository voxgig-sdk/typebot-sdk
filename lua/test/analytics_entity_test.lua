...
describe("AnalyticsEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set TYPEBOT_TEST_ANALYTICS_ENTID JSON to run live")
      return
    end
    ...
    -- LOAD
    local analytics_ref01_ent = client:Analytics(nil)
    local analytics_ref01_match_dt0 = {}
    local analytics_ref01_data_dt0_loaded, err = analytics_ref01_ent:load(analytics_ref01_match_dt0, nil)
    assert.is_nil(err)
    assert.is_not_nil(analytics_ref01_data_dt0_loaded)
