...
TypebotSDK._make_feature = _make_feature


function TypebotSDK.new(options)
  local self = setmetatable({}, TypebotSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function TypebotSDK:options_map()
...
function TypebotSDK:get_utility()
  return Utility.copy(self._utility)
end


function TypebotSDK:get_root_ctx()
  return self._rootctx
end


function TypebotSDK:prepare(fetchargs)
...
function TypebotSDK:direct(fetchargs)
...
-- by passing it.
function TypebotSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function TypebotSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "TypebotSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function TypebotSDK:Analytics(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function TypebotSDK:Billing(data)
...
-- Idiomatic facade: client:Folder():list() / client:Folder():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function TypebotSDK:Folder(data)
  local EntityMod = require("entity.folder_entity")
  if data == nil then
    if self._folder == nil then
      self._folder = EntityMod.new(self, nil)
    end
    return self._folder
  end
  return EntityMod.new(self, data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function TypebotSDK:Result(data)
...
-- Idiomatic facade: client:Typebot():list() / client:Typebot():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function TypebotSDK:Typebot(data)
  local EntityMod = require("entity.typebot_entity")
  if data == nil then
    if self._typebot == nil then
      self._typebot = EntityMod.new(self, nil)
    end
    return self._typebot
  end
  return EntityMod.new(self, data)
end
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function TypebotSDK:Workspace(data)
...
function TypebotSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = TypebotSDK.new(sdkopts)
  sdk.mode = "test"
