...
local AnalyticsEntity = {}
AnalyticsEntity.__index = AnalyticsEntity


function AnalyticsEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AnalyticsEntity)
...
function AnalyticsEntity:get_name()
  return self._name
end


function AnalyticsEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AnalyticsEntity.new(self._client, opts)
end
...
function AnalyticsEntity:mark_deleted()
  self._deleted = true
end


function AnalyticsEntity:deleted()
  return true == self._deleted
end


function AnalyticsEntity:data_set(args)
  if args ~= nil then
...
function AnalyticsEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AnalyticsEntity:match_set(args)
  if args ~= nil then
...
function AnalyticsEntity:match_get()
...
function AnalyticsEntity:stream(action, args, callopts)
...
function AnalyticsEntity:_run_op(ctx, post_done)
  local utility = self._utility
