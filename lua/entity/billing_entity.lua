...
local BillingEntity = {}
BillingEntity.__index = BillingEntity


function BillingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BillingEntity)
  self._name = "billing"
  self._client = client
...
function BillingEntity:get_name()
  return self._name
end


function BillingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BillingEntity.new(self._client, opts)
end
...
function BillingEntity:mark_deleted()
  self._deleted = true
end


function BillingEntity:deleted()
  return true == self._deleted
end


function BillingEntity:data_set(args)
  if args ~= nil then
...
function BillingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BillingEntity:match_set(args)
  if args ~= nil then
...
function BillingEntity:match_get()
...
function BillingEntity:stream(action, args, callopts)
...
---@param reqmatch BillingLoadMatch
---@param ctrl? table
---@return Billing
---@return string? err
function BillingEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch BillingListMatch
---@param ctrl? table
---@return Billing[]
---@return string? err
function BillingEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function BillingEntity:_run_op(ctx, post_done)
  local utility = self._utility
