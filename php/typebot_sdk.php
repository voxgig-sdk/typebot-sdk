...
class TypebotSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $utility = new TypebotUtility();
        $this->_utility = $utility;

        $config = TypebotConfig::shared_config();
        ...
        $feature_opts = TypebotHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = TypebotHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!TypebotFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, TypebotFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = TypebotHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new TypebotError($op . "_allow",
                "TypebotSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = TypebotHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new TypebotError("graphql_error",
                "TypebotSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    // Canonical facade: $client->Folder()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->folder()
    // resolves here too.
    public function Folder($data = null)
    {
        require_once __DIR__ . '/entity/folder_entity.php';
        if ($data === null) {
            if ($this->_folder === null) {
                $this->_folder = new FolderEntity($this, null);
            }
            return $this->_folder;
        }
        return new FolderEntity($this, $data);
    }
    ...
    private $_typebot = null;

    // Canonical facade: $client->Typebot()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->typebot()
    // resolves here too.
    public function Typebot($data = null)
    {
        require_once __DIR__ . '/entity/typebot_entity.php';
        if ($data === null) {
            if ($this->_typebot === null) {
                $this->_typebot = new TypebotEntity($this, null);
            }
            return $this->_typebot;
        }
        return new TypebotEntity($this, $data);
    }
