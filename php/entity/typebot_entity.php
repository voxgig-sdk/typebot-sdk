...
class TypebotEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new TypebotEntity($this->_client, $opts);
    }

    /**
     * @param Typebot|array $args Typebot data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = TypebotHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Typebot|array The current Typebot data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Typebot fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = TypebotHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Typebot fields).
     */
    ...
    /**
     * Load a single Typebot.
     *
     * @param TypebotLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed TypebotLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Typebot|array The loaded Typebot as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = TypebotHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List Typebot items matching the given filter.
     *
     * @param TypebotListMatch|array|null $reqmatch Match filter (any subset
     *   of Typebot fields) as an assoc-array; TypebotListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Typebot[]|array A list of Typebot items as assoc-arrays at
     ...
     *
     * @param TypebotCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed TypebotCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Typebot|array The created Typebot as an assoc-array at the
     *   SDK boundary; throws TypebotError on failure (item-5 convention).
     */
    ...
    /**
     * Update an existing Typebot.
     *
     * @param TypebotUpdateData|array|null $reqdata Body data as an assoc-array;
     *   a typed TypebotUpdateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Typebot|array The updated Typebot as an assoc-array at the
     *   SDK boundary; throws TypebotError on failure (item-5 convention).
     */
    ...
    /**
     * Remove an Typebot matching the given criteria.
     *
     * @param TypebotRemoveMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; TypebotRemoveMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Typebot|array The removed Typebot as an assoc-array at the
     *   SDK boundary; throws TypebotError on failure (item-5 convention).
     */
