...
// Typebot SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class TypebotMakeError
{
    public static function call(?TypebotContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new TypebotContext([], null);
        }
        $op = $ctx->op ?? new TypebotOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new TypebotResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof TypebotError) ? $err->msg : (string)$err;
        $msg = "TypebotSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new TypebotError('', $msg, $ctx);
        ...
        if ($err instanceof TypebotError) {
