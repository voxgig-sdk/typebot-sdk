...
{
    private static function utility(): TypebotUtility
    {
        return new TypebotUtility();
    }
    ...
    private static function ctx(array $over = []): TypebotContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new TypebotContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new TypebotOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof TypebotError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new TypebotOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = TypebotMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new TypebotOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = TypebotMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new TypebotOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = TypebotMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TypebotSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = TypebotMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new TypebotResponse([]),
            'result' => new TypebotResult([]),
        ]);
        [, $err] = TypebotMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new TypebotSpec([]),
            'response' => null,
            'result' => new TypebotResult([]),
        ]);
        [, $err] = TypebotMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new TypebotSpec(['step' => 's']),
            'response' => new TypebotResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new TypebotResult([]),
        ]);
        [, $err] = TypebotMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new TypebotSpec(['step' => 's']),
            'response' => new TypebotResponse(self::resp(200, ['v' => 1])),
            'result' => new TypebotResult([]),
        ]);
        [, $err] = TypebotMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new TypebotSpec(['step' => 's']),
            'response' => new TypebotResponse(self::resp(200, ['v' => 2])),
            'result' => new TypebotResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        TypebotMakeResponse::call($ctx);
    ...
    {
        $preset = new TypebotResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new TypebotSpec([]),
            'response' => new TypebotResponse([]),
            'result' => new TypebotResult([]),
        ...
        [$got, $err] = TypebotMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new TypebotResult([])]);
        [, $err] = TypebotMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new TypebotSpec([]), 'result' => null]);
        [, $err] = TypebotMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TypebotSpec(['step' => 's']),
            'result' => new TypebotResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new TypebotOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TypebotMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TypebotSpec(['step' => 's']),
            'result' => new TypebotResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new TypebotOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TypebotMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TypebotResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new TypebotSpec([]),
            'result' => new TypebotResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = TypebotMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TypebotMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new TypebotError('boom', 'boom');
        $utility->fetcher = function (TypebotContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TypebotSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = TypebotMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (TypebotContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = TypebotMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(TypebotResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (TypebotContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TypebotSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = TypebotMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new TypebotResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new TypebotSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = TypebotMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = TypebotMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = TypebotMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new TypebotResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, TypebotDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new TypebotResult(['ok' => false])]);
        $this->expectException(TypebotError::class);
        TypebotDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new TypebotResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', TypebotMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        TypebotMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new TypebotBaseFeature();
        $b = new TypebotBaseFeature();
        TypebotFeatureAdd::call($ctx, $a);
        TypebotFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): TypebotBaseFeature
    {
        $f = new TypebotBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        TypebotFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): TypebotContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new TypebotSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TypebotPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new TypebotResponse(['headers' => null]),
            'result' => new TypebotResult([]),
        ]);
        TypebotResultHeaders::call($ctx);
