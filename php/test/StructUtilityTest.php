...
// Typebot SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = TypebotOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(TypebotOmni::structfix($now, Struct::undef()))
                        !== json_encode(TypebotOmni::structfix($val, Struct::undef()))) {
