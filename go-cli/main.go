// Command Typebot-cli is a CLI + REPL for the Typebot SDK,
...
const prompt = "typebot"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "analytics billing folder result typebot workspace"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      TYPEBOT_APIKEY (api key), TYPEBOT_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: TYPEBOT_APIKEY carries the API key and
	// TYPEBOT_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("TYPEBOT_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("TYPEBOT_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTypebotSDK(opts)
